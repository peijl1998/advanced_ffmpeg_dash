...
static void free_video_list(DASHContext *c)
{
    int i;
                                                ...
                                                xmlNodePtr period_segmentlist_node)
        {
            ...
            ret = parse_manifest_adaptationset_attr(s, pd, adaptionset_node);
            if (ret < 0)
                return ret;
                ...
                static int parse_manifest_period(AVFormatContext* s,
                    ...
                    while (attr) {
                        ...
                        if (!av_strcasecmp(attr->name, "duration")) {
                            pd->period_duration_sec = get_duration_insec(s, val);
                        } else if (!av_strcasecmp(attr->name, "start")) {
                            pd->period_start_sec = get_duration_insec(s, val);
                        }
                        attr = attr->next;
                        xmlFree(val);
                    }
...
static int dash_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i = 0; i < c->periods[c->current_periodc->periods[c->current_period]->n_audios; i++) {
        rep = c->periods[c->current_period]->audios[i];
        if (!rep->ctx)
            continue;
        if (!cur || rep->cur_timestamp < mints) {
