...
#include "libavutil/dashdec_segmentbase_parser.h"
#include "internal.h"
...
static int dashdec_get_stream(AVFormatContext* s, enum AVMediaType type) {
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        av_log(c, AV_LOG_ERROR, "mediatype not found when getting stream");
    }

    
    if (type == AVMEDIA_TYPE_VIDEO) {
        size = c->periods[c->current_period]->n_videos;
...
static int is_handle_type(xmlNodePtr node, const char* mode) {
    char *val = NULL;
    if (node) {
        val = xmlGetProp(node, "mimeType");
        if (val) {
            if (av_stristr(val, mode)) {
                return 1;
            }
        }
    }
                                         ...
                                         xmlNodePtr adaptionset_supplementalproperty_node)
{
    ...
    if (!rep)
        return AVERROR(ENOMEM);
    if (c->periods[c->n_periods - 1]->adaptionset_lang) {
        rep->lang = av_strdup(c->periods[c->n_periods - 1]->adaptionset_lang);
        if (!rep->lang) {
    ...
    if (representation_segmenttemplate_node || fragment_template_node || period_segmenttemplate_node) {
        ...
        // BUPT
        int handle_ret = -1;
        char* handle_url = get_content_url(baseurl_nodes, 4, c->max_url_size,
                                           rep->id, rep_bandwidth_val, NULL);
        if (is_handle_type(adaptionset_node, "webm") || is_handle_type(representation_node, "webm")) {
            handle_ret = handle_webm_segmentbase(handle_url, c, rep, representation_segmentbase_node);
        } else if (is_handle_type(adaptionset_node, "mp4") || is_handle_type(representation_node, "mp4")) {
            handle_ret = handle_fmp4_segmentbase(handle_url, c, rep, representation_segmentbase_node);
        }
        /////////////
        if (handle_ret != 0) {
    ...
    case AVMEDIA_TYPE_VIDEO:
        ret = av_dynarray_add_nofree(&c->periods[c->current_period]->videos, &c->periods[c->n_periods - 1]->n_videos, rep);
        break;
    case AVMEDIA_TYPE_AUDIO:
        ret = av_dynarray_add_nofree(&c->periods[c->current_period]->audios, &c->periods[c->n_periods - 1]->n_audios, rep);
        break;
    case AVMEDIA_TYPE_SUBTITLE:
        ret = av_dynarray_add_nofree(&c->periods[c->current_period]->subtitles, &c->periods[c->n_periods - 1]->n_subtitles, rep);
        break;
    }
    if (ret < 0)
        goto free;
                                             ...
                                             xmlNodePtr adaptionset_node)
{
    ...
    c->periods[c->n_periods - 1]->adaptionset_lang = xmlGetProp(adaptionset_node, "lang");
                                        ...
                                        xmlNodePtr period_segmentlist_node)
{
    ...
    if (c->n_periods == 0) {
        av_log(s, AV_LOG_ERROR, "Cannot get Periods\n");
        return AVERROR(EINVAL);
    }
...
err:
    xmlFree(c->periods[c->current_period]->adaptionset_lang);
...
static int parse_manifest_period(AVFormatContext* s,
    ...
    while (attr) {
        ...
        if (!av_strcasecmp(attr->name, "duration")) {
            pd->period_duration = get_duration_insec(s, val);
        } else if (!av_strcasecmp(attr->name, "start")) {
            pd->period_start = get_duration_insec(s, val);
        }
        attr = attr->next;
        xmlFree(val);
    }
    ...
    c->current_period = c->n_periods - 1;
...
static int dash_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    recheck_discard_flags(s, c->periods[c->current_period]->videos, c->periods[c->current_period]->n_videos);
    recheck_discard_flags(s, c->periods[c->current_period]->audios, c->periods[c->current_period]->n_audios);
    recheck_discard_flags(s, c->periods[c->current_period]->subtitles, c->periods[c->current_period]->n_subtitles);
...
static int handle_webm_segmentbase(char* url, DASHContext* c, struct representation* rep, xmlNodePtr base) {
    ...
    init_seg->size += 1; // note: mpd generated by ffmpeg-webmdash maybe wrong and need plus 1.
...
static int handle_fmp4_segmentbase(char* url, DASHContext* c, struct representation* rep, xmlNodePtr base) {
    AVDictionary* opts = NULL;
    AVFormatContext *init = NULL;
    xmlNodePtr init_node = NULL;
    struct fragment* init_seg = NULL, *seg;
    char range_val[50];
    int err;
    uint64_t moov_end = -1;
    ...
    init_seg = get_Fragment(xmlGetProp(init_node, "range"));
    init_seg->url = url;
    av_dict_set_int(&opts, "offset", init_seg->url_offset, 0);
    av_dict_set_int(&opts, "end_offset", init_seg->url_offset + init_seg->size, 0);
    av_dict_copy(&opts, c->avio_opts, 0);
    

    init = avformat_alloc_context();
    avio_open2(&init->pb, url, AVIO_FLAG_READ, c->interrupt_callback, &opts);
    err = dashdec_fmp4_parse_init(init->pb, &seg_head, &moov_end);
    if (err < 0 || !seg_head) {
        free(init_seg);
        free(init);
        dashdec_fmp4_free(seg_head);
        return -1;
    }

    cursor = seg_head;
    while (cursor) {
        sprintf(range_val, "%lu-%lu", cursor->begin, cursor->begin + cursor->size);
        seg = get_Fragment(range_val);
        if (!seg) {
            return -1;
        }
        seg->duration = cursor->duration;
        seg->url = url;
        err = av_dynarray_add_nofree(&rep->fragments, &rep->n_fragments, seg);
        if (err < 0) {
            free_fragment(&seg);
